...
// Shoutouts to Epyx for finding a bug
...
init
{
    ...
    if (states.TryGetValue(modules.First().ModuleMemorySize, out memoryOffset))
        if (memory.ProcessName.ToLower().Contains("snes9x"))
            memoryOffset = memory.Is64Bit()
                ? (long)memory.ReadPointer((IntPtr)memoryOffset)
                : memory.ReadValue<int>((IntPtr)memoryOffset);
    ...
    vars.watchers = new MemoryWatcherList
    {
		...
		new MemoryWatcher<byte>((IntPtr)memoryOffset + 0x1E696) { Name = "new_floor" },
    	new MemoryWatcher<byte>((IntPtr)memoryOffset + 0x421d) { Name = "blob" },
		new MemoryWatcher<byte>((IntPtr)memoryOffset + 0x05AC) { Name = "map_ID" },
...
split
{
    var nextfloor = settings["EveryFloor"] && (vars.watchers["new_floor"].Old + 1 == vars.watchers["new_floor"].Current) && vars.watchers["new_floor"].Current != 1;
    var tenfloor = settings["Every10th"] && (vars.watchers["new_floor"].Old + 1 == vars.watchers["new_floor"].Current) && (((vars.watchers["new_floor"].Current - 2) % 10) == 0) && vars.watchers["new_floor"].Current !=1;
    var first = settings["1st"] && (vars.watchers["new_floor"].Old + 1 == vars.watchers["new_floor"].Current) && vars.watchers["new_floor"].Current == 2;
    var nineeight = settings["98th"] && vars.watchers["new_floor"].Current == 99 && vars.watchers["map_ID"].Old == 240 && vars.watchers["map_ID"].Current == 241;
    var blobkill = settings["Blobkill"] && vars.watchers["blob"].Current == 31 && vars.watchers["blob"].Old == 00 && vars.watchers["new_floor"].Current == 99 && vars.watchers["map_ID"].Current == 241 && (vars.watchers["round_counter"].Current == 1 || vars.watchers["round_counter"].Current == 2 || vars.watchers["round_counter"].Current == 3);
